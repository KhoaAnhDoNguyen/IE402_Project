...
function CardRent({ item, onDelete }) {
  ...
  const { name: wardName = "Ward", districts = {} } = wards;
  const { name: districtName = "District" } = districts;

  return (
    <div className="card">
      <Link to={`/${id}`} className="imageContainer">
        <img src={images[0]?.image_url || "/default-image.png"} alt={name} />
      </Link>
      <div className="textContainer">
        <h2 className="title">
        ...
        <p className="address">
          <img src="/pin.png" alt="Location" />
          <span>
            {street}, {wardName}, {districtName}
          </span>
        </p>
        <p className="price">
          {price ? `${price.toLocaleString()} VND` : "Liên hệ để biết giá"}
        </p>
        <p className="rent-month">
          Tháng thuê: {item.rent_month || "Không xác định"}
        </p>
